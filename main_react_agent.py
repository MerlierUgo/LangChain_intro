...
def get_text_length(text:str)-> int:
    ...
    text = text.strip("'\n").strip('"') # strip away non alphabetic characters just in case
    return len(text)
...
def main():
    ...
    # choisit depuis langsmitch prompt hwchase17/react
    template = """
    ...
    #llm = ChatAnthropic(temperature=0, model_name="claude-3-haiku-20240307", stop="\nObservation")
    #llm = ChatAnthropic(temperature=0, model_name="claude-3-haiku-20240307", model_kwargs={"stop" : ["\nObservation", "Observation"]}, callbacks=[AgentCallbackHandler()])
    llm = ChatOpenAI(
        model="gpt-4o", 
        temperature=0,
